...
<!--商品卡-->
<% 
							sql="select * from product where pid='5'";
	...
	<form method="get" action="cart_update.jsp">				
		...
		try {
			...
			sql="SELECT * FROM `board` where pid='5'"; //算出共幾筆留言
				...
				if (current_page > 1)
				{
					out.println("<a href='5.jsp?page=1'>第一頁</a>&nbsp;");
					out.println("<a href='5.jsp?page="+(current_page-1)+ "'>上一頁</a>&nbsp;");						
				}
				//使用超連結方式, 呼叫自己, 使用get方式傳遞參數(變數名稱為page)
				for(int i=1;i<=page_num;i++) //建立1,2,...頁超連結
					out.print("<a href='5.jsp?page="+i+"'>"+i+"</a>&nbsp;"); //&nbsp;html的空白
				
				if (current_page < page_num)
				{
					out.println("<a href='5.jsp?page="+(current_page+1)+"'>下一頁</a>&nbsp;");
					out.println("<a href='5.jsp?page="+page_num+"'>最後一頁</a>");						
				}
				out.print("，目前在第"+current_page+"頁");				
				out.println("<br>");
					
				out.println("<form action='5.jsp' method='get'  style='padding: 10px 0;'>");
				out.println("跳至第");
				...
				//遞減排序, 讓最新的資料排在最前面
				sql="SELECT * FROM `board` where pid='5' ORDER BY `DATE` DESC LIMIT ";
				sql+=start_record+",5";
